...
public class DTWithBigPoints {
	...
	public DTWithBigPoints(List<Point> points){
		...
		bigPoints[0] = new Vertex(new Point(-4000,-4000)); 
		bigPoints[1] = new Vertex(new Point( 4000, 0));
		bigPoints[2] = new Vertex(new Point( 0, 4000));
		Triangle bigTri = new Triangle(bigPoints[0], bigPoints[1], bigPoints[2]);
		triangles.add(bigTri);
		for(Point p: points){
			p.addThis(new Vector(Randomization.randBetween(-0.00001, 0.00001), Randomization.randBetween(-0.00001, 0.00001)));
			addPoint(p);
		}
	}
