...
public class AlphaComplex extends AlphaFiltration implements SimplicialComplex{
	...
	public static void main(String[] args) {
		...
		PDBFile f = new PDBFile(PDBWebReader.downloadPDBFile("1CRN"));
		List<Point> atoms = f.getAtomCoords();
		AlphaComplex ac = new AlphaComplex(atoms, 2.3
				);
		J3DScene scene = J3DScene.createJ3DSceneInFrame();
//		for(CTriangle t: ac.getTriangles()){
//			scene.addShape(t, new Color(255,80,50));
//		}
//		for(CTetrahedron t: ac.getTetrahedra()){
//			scene.addShape(t, new Color(255,30,50));
//		}
		for(PDBFile.AtomRecord ar: f.getAtomRecords()){
			Color c = Color.GRAY;
			if(ar.element.equalsIgnoreCase("H")) c = Color.WHITE;
			if(ar.element.equalsIgnoreCase("N")) c = Color.BLUE;
			if(ar.element.equalsIgnoreCase("O")) c = Color.RED;
			scene.addShape(new ProGAL.geom3d.volumes.Sphere(ar.coords,0.4), c, 9);
		}
		for(CEdge e: ac.getEdges(2.3)){
			scene.addShape(new ProGAL.geom3d.volumes.LSS(e.getA(), e.getB(), 0.04), Color.GRAY.brighter(), 4);
		}
