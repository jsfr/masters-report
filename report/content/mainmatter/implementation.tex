...
The new implementation has been written in the, still relatively new, language
Go, which quoting the homepage: ``Go is an open source programming language that
...
Finally the easy use of concurrency in Go\footnote{Starting a new goroutine
  is as simple as writing: \texttt{go~SomeFunction~(args\ldots)}.} made it interesting as potential speedup could be achieved by
making the the branching of the program concurrent.
...
In the code a optimization method using the analytical method has been
...
equations\footnote{Note that all the following calculations can also be found
  in~\cite{uteshev2014}, however as these are a bit convoluted they are also
...
contrast to not sorting the terminals.\NOTE{Maybe give some reason for
  suggesting another method, like it is simpler?} Here another method is
