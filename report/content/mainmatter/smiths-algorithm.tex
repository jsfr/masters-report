...
\item $L$, the length of the tree. Defined as the sum of the euclidean lengths of
  all edges in in the topology $T$. See \Cref{eq:length} for the exact definition.
\end{itemize}

\FIXME[inline]{Smith is inconsistent with having the sets as containing either
  the points of the indicies. This we should probably fix up just to clear the notation.}
...
\begin{equation}
  \label{eq:6}
  \vec x_k^{(i+1)} = \left. \sum_{j : (j, k) \in T} \frac{\vec
      x_j^{(i+1)}}{|\vec x_j^{(i)} - \vec x_k^{(i)}|} \hspace{0.5em}\middle/\hspace{-0.5em} \sum_{j: (j,k)
...
\begin{align}
  ...
  0                & = \vec a \cdot \vec b +
                     \frac{1}{2} (|\vec a| \cdot |\vec b|) \Leftrightarrow \label{eq:4} \\
...
\begin{table}[htbp]
  ...
  \caption[Angles and their corresponding ``error functions'']{Angles smaller
    $\mathit{120^{\circ}}$ clearly means that the error function becomes positive, whereas
    those greater than $\mathit{120^{\circ}}$ will become negative.\label{tab:error-functions}}
\end{table}

Note however that while it is now clear that the function is dependent on the length
of the edges, it is still somewhat unclear how it exactly relates to the entire
length of the tree, and why we are looking at $E \ll L$ instead of $E <
