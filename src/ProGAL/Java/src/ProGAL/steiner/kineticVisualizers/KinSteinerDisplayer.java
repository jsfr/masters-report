...
public class KinSteinerDisplayer implements ClickListener{
	...
	private void animate(){
		int p1 = 1, p2 = 2;
		Point c1 = new Point(pp.points[p1]).add(new Vector(0,0));
		Point c2 = new Point(pp.points[p2]).add(new Vector(0,0));
		double r1 = 0.3;
		double r2 = 0.8;
		for(double t=0;;t+=0.1){
			Point pos1 = c1.add(new Vector(1,0).multiplyThis(r1*Math.cos(t+3.141592/2)));
			pos1.addThis(new Vector(0,1).multiplyThis(r1*Math.sin(t+3.141592/2)));
			pp.points[p1].set(pos1);
			
			Point pos2 = c2.add(new Vector(1,0).multiplyThis(r1*Math.cos(-t)));
			pos2.addThis(new Vector(0,1).multiplyThis(r1*Math.sin(-t)));
			pp.points[p2].set(pos2);
...
//		}
	...
	public static void main(String[] args) {
		ProGAL.geomNd.Point[] sites = new ProGAL.geomNd.Point[]{
				...
				new ProGAL.geomNd.Point(new double[]{0.3,1}),
				new ProGAL.geomNd.Point(new double[]{1.8,0.7}),
				new ProGAL.geomNd.Point(new double[]{1.2,0.9}),
				new ProGAL.geomNd.Point(new double[]{1.5,-0.5}),
//				new ProGAL.geomNd.Point(new double[]{0,-0.5}),
//				new ProGAL.geomNd.Point(new double[]{3,0}),
//				new ProGAL.geomNd.Point(new double[]{3,1}),
//				new ProGAL.geomNd.Point(new double[]{3,2}),
//				new ProGAL.geomNd.Point(new double[]{3,3}),
//				new ProGAL.geomNd.Point(new double[]{3,4}),
//				new ProGAL.geomNd.Point(new double[]{3,5}),
//				new ProGAL.geomNd.Point(new double[]{3,0}),
		}; 
		KinSteinerDisplayer dis = new KinSteinerDisplayer(sites);
//		dis.animate();
	}
