...
static REAL D[NMAX][NMAX];
static int MSTA[NMAX-1], MSTB[NMAX-1];
...
void output_mst_tree_to_maple()
{
  ...
  for (i = 1; i < NUMSITES; i++) 
    printf("POLYGONS([[%g, %g, %g], [%g, %g, %g]]),\n",
	   XX[MSTA[i]][0], XX[MSTA[i]][1], XX[MSTA[i]][2],
	   XX[MSTB[i]][0], XX[MSTB[i]][1], XX[MSTB[i]][2]);
  printf("AXESSTYLE(FRAME));\n");
}
...
{ /* Inputs NUMSITES, DIMENSION, sites; outputs succesive best Steiner
  ...
  int i, j, x, k, m, nc, ct;  REAL q, r, t; int A[NMAX];
     REAL MaxReal=999999999.9;
     static int OUT[NMAX];
     static REAL MIND[NMAX];
     REAL MinDist;
     int MinJ;
  INITRAND();
  ...
  if (NUMSITES == 3) { /* Deal with special case of 3 sites */
    ...
    printf("\nnew record length %20.20g, ratio %6.6g\n", q, q/MSTLength);
    output_tree();
  ...
  /* MST computation */
     ...
     MSTLength=0.0;
     for (j = 2; j <= NUMSITES; j++) { MIND[j]=D[1][j]; OUT[j]=1; }
     for (i = 1; i < NUMSITES; i++)
     { 
       MinJ=0; MinDist=9999999999.9;
       for (j = 2; j <= NUMSITES; j++)
       { if (OUT[j] > 0)
           if (MIND[j] < MinDist) { MinDist=MIND[j]; MinJ=j; }
       }
       MSTA[i]=OUT[MinJ];
       MSTB[i]=MinJ;
       MSTLength += D[OUT[MinJ]][MinJ];
       OUT[MinJ]=0;
